...
if [ $ROSVERSION_INSTALLED -eq 0 ]; then # we need to install ROS
    ...
    $ABSOLUTE_PATH/check_pkg_status_and_install.sh python-rosinstall # indigo and jade only need this; kinetic and melodic need this plus what is below
    if [ $ROSVERSION == "kinetic" ] || [ $ROSVERSION == "melodic" ]; then
        $ABSOLUTE_PATH/check_pkg_status_and_install.sh python-rosinstall-generator python-wstool build-essential
    fi
fi
